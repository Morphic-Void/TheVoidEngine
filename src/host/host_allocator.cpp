...
class CSystemAllocator : public memory::IAllocator
{
public:
    ...
    {
        return (bytes != 0u) ? ::operator new[](bytes, memory::alignment_policy(align), std::nothrow) : nullptr;
    }

    virtual bool byte_deallocate(void* const ptr, const std::size_t align) noexcept override final
    {
        if (ptr != nullptr)
        {
            ::operator delete[](ptr, memory::alignment_policy(align));
            return true;
        }
        return false;
    }
...
memory::IAllocator* get_the_host_allocator(const std::size_t system_id) noexcept
{
